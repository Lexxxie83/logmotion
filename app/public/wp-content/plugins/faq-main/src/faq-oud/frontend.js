...
class Collapse {
	...
	_setPanelHeight( panel ) {
		const contents = panel.querySelector( 'summary + *' );
	...
	_attachEvents() {
		this._panels.forEach( panel => {
			const toggler = panel.querySelector( 'summary' );
			const contents = panel.querySelector( 'summary + *' );

			// On panel open
			panel.addEventListener( 'toggle', e => {
				const isReadingHeight = panel.classList.contains( this.settings.heightClass );
      ...
      */
			...
			contents.addEventListener( 'transitionend', ( e ) => {
				...
				if ( e.propertyName == propToWatch ) {
					const wasOpened = panel.classList.contains( this.settings.activeClass );
...
// const makeMePretty = document.querySelector( '.collapse' );

// if ( makeMePretty ) {
// 	const accordion = new Collapse( makeMePretty, { accordion: true } ).init();
// }
...
[].forEach.call(faqs, function(faq) {
	// do whatever
	new Collapse( faq, { accordion: true } ).init();
  });
